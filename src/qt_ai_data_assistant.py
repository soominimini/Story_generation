...
class QTAIDataAssistant(ParamifyWeb, BaseNode):
    ...
    def __init__(self, 
        ...
        # Initialize user management before BaseNode
        self.user_manager = UserManager()
        self.user_interface = UserCLIInterface(self.user_manager)
        
        # Initialize BaseNode (this will call setup())
        BaseNode.__init__(self, paused=self.parameters.paused)


    def setup(self):
        # Handle user authentication first
        print(colored("🤖 QTrobot AI Assistant - User Authentication", 'green'))
        print(colored("="*50, 'green'))
        
        # Run user interface to get authenticated user
        if not self.user_interface.run():
            rospy.logerr("User authentication failed. Exiting...")
            rospy.signal_shutdown("User authentication failed")
            return
        
        # Get current user info
        current_user = self.user_manager.get_current_user()
        if current_user:
            rospy.loginfo(f"User authenticated: {current_user['username']} (Age: {current_user['age']})")
    ...
    def _reset_chat_engine(self):
        ...
        user_mem_store = self.user_manager.get_user_mem_store_path()
        if user_mem_store:
            rospy.loginfo(f"Using user-specific memory store: {user_mem_store}")
        else:
            rospy.logwarn("No user logged in, using default memory store")
            user_mem_store = self.parameters.mem_store
    ...
    def _asr_callback(self, text, language):     
        ...
        current_user = self.user_manager.get_current_user()
        if current_user:
            rospy.loginfo(f"Logged-in user {current_user['username']} (Age: {current_user['age']}) says: {text}")
            # Include user info in the chat context
            user_context = f"{current_user['username']} (Age: {current_user['age']})"
        else:
        ...
        try:      
            ...
            response_stream = self.chat.get_stream_response(text=text, user_id=user_context)
